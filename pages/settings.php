        ...
        <div class="mt-5 grid gap-5 lg:grid-cols-[0.74fr_1.26fr]">
            <aside class="settings-card rounded-[28px] border-4 border-arcade-ink bg-arcade-panel p-5 shadow-[8px_8px_0_#26190f]">
                ...
                <div class="mt-5 flex flex-col items-center text-center">
                    <div class="settings-avatar grid h-32 w-32 place-items-center overflow-hidden rounded-[32px] border-4 border-arcade-ink bg-arcade-yellow shadow-[7px_7px_0_rgba(38,25,15,0.24)]" aria-label="Current avatar preview">
                        <?php if ($profileAvatarUrl !== '') : ?>
                            <img id="settings-current-avatar" src="<?= htmlspecialchars($profileAvatarUrl, ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($profileName, ENT_QUOTES, 'UTF-8') ?> avatar" class="h-full w-full object-cover">
                        <?php else : ?>
                            <span id="settings-current-avatar-initials" class="font-arcade text-3xl text-arcade-ink"><?= htmlspecialchars($profileAvatarInitials, ENT_QUOTES, 'UTF-8') ?></span>
                        <?php endif; ?>
                    </div>
            ...
            <form class="settings-form rounded-[28px] border-4 border-arcade-ink bg-arcade-panel p-5 shadow-[8px_8px_0_#26190f] md:p-6" action="./?c=settings" method="post" enctype="multipart/form-data">
                <?= pixelwarCsrfField() ?>
                    ...
                    <label class="settings-upload sm:col-span-2" for="settings-avatar-file">
                        ...
                        <div class="settings-upload__body">
                            <div class="settings-upload__preview">
                                <?php if ($profileAvatarUrl !== '') : ?>
                                    <img id="settings-avatar-preview" src="<?= htmlspecialchars($profileAvatarUrl, ENT_QUOTES, 'UTF-8') ?>" alt="" class="h-full w-full object-cover">
                                <?php else : ?>
                                    <span id="settings-avatar-preview-initials" class="font-arcade text-sm"><?= htmlspecialchars($profileAvatarInitials, ENT_QUOTES, 'UTF-8') ?></span>
                                    <img id="settings-avatar-preview" src="" alt="" class="hidden h-full w-full object-cover">
                                <?php endif; ?>
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-black text-arcade-ink">Upload new avatar</p>
                                <p id="settings-avatar-file-name" class="mt-1 truncate text-xs font-bold text-arcade-ink/58">PNG, JPG, WEBP, or GIF. Max 2MB.</p>
                            </div>
                            <strong class="settings-upload__button">Choose File</strong>
                        </div>
                        <input id="settings-avatar-file" name="profile_image" type="file" accept="image/png,image/jpeg,image/webp,image/gif" class="sr-only">
                    </label>

                    <label class="settings-field" for="settings-firstname">
                        <span>First Name</span>
                        <input id="settings-firstname" name="firstname" type="text" autocomplete="given-name" maxlength="80" value="<?= htmlspecialchars($profileFirstname, ENT_QUOTES, 'UTF-8') ?>" placeholder="Melvin" required>
                    </label>

                    <label class="settings-field" for="settings-lastname">
                        <span>Last Name</span>
                        <input id="settings-lastname" name="lastname" type="text" autocomplete="family-name" maxlength="80" value="<?= htmlspecialchars($profileLastname, ENT_QUOTES, 'UTF-8') ?>" placeholder="Agustin" required>
                    </label>

                    <label class="settings-field sm:col-span-2" for="settings-email">
                        <span>Email</span>
                        <input id="settings-email" name="email" type="email" autocomplete="email" value="<?= htmlspecialchars($profileEmail, ENT_QUOTES, 'UTF-8') ?>" data-current-email="<?= htmlspecialchars($profileEmail, ENT_QUOTES, 'UTF-8') ?>" placeholder="player@example.com" required>
