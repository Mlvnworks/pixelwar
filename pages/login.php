...
<main class="login-page relative min-h-[calc(100vh-4.25rem)] overflow-hidden bg-arcade-cream px-4 py-4 text-arcade-ink">
    <div
        class="login-bg absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(76,201,240,0.2),transparent_24%),radial-gradient(circle_at_80%_80%,rgba(255,209,102,0.3),transparent_26%)]">
    </div>
    ...
    <section class="container relative flex min-h-[calc(100vh-7.25rem)] items-center justify-center">
        <form id="login-form"
            class="login-card w-full max-w-[23.5rem] rounded-[24px] border-4 border-arcade-ink bg-arcade-panel p-4 shadow-[8px_8px_0_#26190f] md:p-5"
            action="./?c=login" method="post" novalidate>
            <?= pixelwarCsrfField() ?>
            ...
            <p class="mt-1.5 text-sm leading-5 text-arcade-ink/68">Jump back in with your Pixelwar account or continue
                with Google.</p>

            <?php if ($loginErrors !== []): ?>
                <div class="mt-3 rounded-2xl border-2 border-arcade-coral bg-arcade-coral/10 px-3 py-2 text-sm font-bold leading-5 text-arcade-ink"
                    role="alert">
            ...
            <input id="login-identity" name="identity" type="text" autocomplete="username" required
                value="<?= htmlspecialchars((string) ($loginOld['identity'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
                class="mt-1 w-full rounded-xl border-2 border-arcade-ink/15 bg-white px-3 py-2 outline-none transition focus:border-arcade-orange"
            ...
            <div class="mt-2 text-right">
                <a href="./?c=forgot-password"
                    class="text-xs font-bold text-arcade-orange no-underline hover:text-arcade-coral">Forgot
                    password?</a>
            </div>

            <button id="login-submit-button" type="submit"
                class="auth-submit-button mt-4 inline-flex w-full items-center justify-center gap-3 rounded-xl border-2 border-arcade-ink bg-arcade-yellow px-6 py-2 text-sm font-bold shadow-[0_4px_0_#26190f] transition hover:-translate-y-0.5 hover:bg-arcade-orange hover:text-white">
                <span
                    class="auth-submit-spinner hidden h-4 w-4 rounded-full border-2 border-arcade-ink/40 border-t-arcade-ink"
            ...
            <p class="mt-3 text-center text-sm text-arcade-ink/68">
                New player?
                <a href="./?c=signup" class="font-bold text-arcade-orange no-underline hover:text-arcade-coral">Create
                    an account</a>
            </p>
        </form>
    </section>
