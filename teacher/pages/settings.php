...
$teacherPasswordResetAvailableAt = (int) ($_SESSION['teacher_password_reset_available_at'] ?? 0);
$teacherPasswordResetSecondsLeft = max(0, $teacherPasswordResetAvailableAt - time());
...
<main class="teacher-shell relative overflow-hidden px-4 py-6 text-arcade-ink md:py-8">
    ...
    <section class="container relative">
        ...
        <div class="mt-5 grid gap-5 lg:grid-cols-[0.74fr_1.26fr]">
            ...
            <form id="teacher-settings-password-reset-form" action="./?c=settings" method="post" class="hidden">
            ...
            <form
                ...
                <section class="mt-6 rounded-2xl border-2 border-arcade-ink/10 bg-white/75 p-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex shrink-0 flex-col items-start gap-2 sm:items-end">
                            <button
                                type="submit"
                                form="teacher-settings-password-reset-form"
                                ...
                                data-teacher-password-reset-button
                                data-teacher-password-reset-available-at="<?= (int) $teacherPasswordResetAvailableAt ?>"
                                <?= $teacherPasswordResetSecondsLeft > 0 ? 'disabled' : '' ?>
                            >
                                <span class="teacher-password-reset-button__spinner hidden h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" aria-hidden="true"></span>
                                <svg class="teacher-password-reset-button__icon h-4 w-4" viewBox="0 0 16 16" aria-hidden="true" focusable="false">
                                    <path fill="currentColor" d="M8 1a4 4 0 0 1 4 4v2h1v8H3V7h1V5a4 4 0 0 1 4-4Zm2 6V5a2 2 0 0 0-4 0v2h4Zm-5 2v4h6V9H5Z" />
                                </svg>
                                <span class="teacher-password-reset-button__text">Send Reset Link</span>
                            </button>
                            <p
                                class="<?= $teacherPasswordResetSecondsLeft > 0 ? '' : 'hidden' ?> text-xs font-bold text-arcade-ink/55"
                                data-teacher-password-reset-countdown
                            >
                                Resend available in <?= (int) $teacherPasswordResetSecondsLeft ?>s.
                            </p>
                        </div>
                    </div>
                </section>
                ...
                <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button type="submit"
                        class="teacher-button teacher-button--primary gap-2 disabled:cursor-not-allowed disabled:opacity-70"
                        data-teacher-settings-save-button>
                        <span class="teacher-settings-save-button__spinner hidden h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" aria-hidden="true"></span>
                        <svg class="teacher-settings-save-button__icon h-4 w-4" viewBox="0 0 16 16" aria-hidden="true" focusable="false">
                            <path fill="currentColor" d="M3 2h8l2 2v10H3V2Zm2 2v3h5V4H5Zm0 6v2h6v-2H5Z" />
                        </svg>
                        <span class="teacher-settings-save-button__text">Save Changes</span>
                    </button>
                </div>
            </form>
        </div>
...
(() => {
    ...
    const saveButton = document.querySelector('[data-teacher-settings-save-button]');
    const saveButtonSpinner = saveButton?.querySelector('.teacher-settings-save-button__spinner');
    const saveButtonIcon = saveButton?.querySelector('.teacher-settings-save-button__icon');
    const saveButtonText = saveButton?.querySelector('.teacher-settings-save-button__text');
    const passwordResetForm = document.querySelector('#teacher-settings-password-reset-form');
    const passwordResetButton = document.querySelector('[data-teacher-password-reset-button]');
    const passwordResetSpinner = passwordResetButton?.querySelector('.teacher-password-reset-button__spinner');
    const passwordResetIcon = passwordResetButton?.querySelector('.teacher-password-reset-button__icon');
    const passwordResetText = passwordResetButton?.querySelector('.teacher-password-reset-button__text');
    const passwordResetCountdown = document.querySelector('[data-teacher-password-reset-countdown]');
    ...
    const setSaveButtonLoading = (isLoading) => {
        if (!saveButton || !saveButtonSpinner || !saveButtonIcon || !saveButtonText) {
            return;
        }

        saveButton.disabled = isLoading;
        saveButtonSpinner.classList.toggle('hidden', !isLoading);
        saveButtonIcon.classList.toggle('hidden', isLoading);
        saveButtonText.textContent = isLoading ? 'Saving...' : 'Save Changes';
        saveButton.setAttribute('aria-busy', isLoading ? 'true' : 'false');
    };
    ...
    const updatePasswordResetCountdown = () => {
        ...
        const availableAt = Number(passwordResetButton.dataset.teacherPasswordResetAvailableAt || 0);
