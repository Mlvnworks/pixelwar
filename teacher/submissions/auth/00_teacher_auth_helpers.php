...
function teacherPanelRequireTools($tools): Tools
{
...
function teacherPanelAppUrl(string $path = ''): string
{
    $https = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off')
        || ((string) ($_SERVER['SERVER_PORT'] ?? '') === '443');
    $scheme = $https ? 'https' : 'http';
    $host = (string) ($_SERVER['HTTP_HOST'] ?? 'localhost');
    $scriptPath = str_replace('\\', '/', dirname(dirname((string) ($_SERVER['SCRIPT_NAME'] ?? '/teacher/index.php'))));
...
function teacherPanelSendPasswordResetLink(Tools $tools, string $email, string $username, string $resetLink): bool
{
    $safeUsername = htmlspecialchars($username, ENT_QUOTES, 'UTF-8');
    ...
    $content = '<!doctype html>'
        ...
        . '<tr><td align="center">'
        . '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#fffdf6;border:4px solid #26190f;border-radius:28px;box-shadow:8px 8px 0 #26190f;overflow:hidden;">'
        . '<tr><td style="padding:24px 24px 18px;background:linear-gradient(135deg,#ffd166 0%,#4cc9f0 52%,#ff8c42 100%);border-bottom:4px solid #26190f;">'
        . '<p style="margin:0 0 12px;font-size:11px;font-weight:900;letter-spacing:4px;text-transform:uppercase;color:#26190f;">Pixelwar Teacher</p>'
        . '<h1 style="margin:0;font-size:28px;line-height:1.12;color:#26190f;">Reset your password</h1>'
        . '</td></tr>'
        . '<tr><td style="padding:26px 24px 24px;">'
        . '<p style="margin:0 0 14px;font-size:16px;line-height:1.65;color:#26190f;">Hello <strong>' . $safeUsername . '</strong>, we received a request to change your Pixelwar teacher password.</p>'
    ...
    $result = $tools->sendEmail($content, $email, 'Reset your Pixelwar teacher password');

    if (empty($result['success'])) {
        $error = $result['err'] ?? null;
        error_log('Teacher password reset email failed: ' . ($error instanceof Throwable ? $error->getMessage() : 'Unknown mail error'));
