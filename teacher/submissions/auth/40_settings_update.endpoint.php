...
if ($teacherRequestMethod === 'POST' && $teacherRequestedPage === 'settings') {
    ...
    $accounts = teacherPanelRequireUserAccountService($userAccountService);
    $verifications = teacherPanelRequireVerificationRepository($verificationRepository);
    ...
    try {
        ...
        if ($emailChanged) {
            $updatedUser = $users->findUserForSettings($userId);

            if (!$updatedUser || (int) ($updatedUser['is_verified'] ?? 1) !== 0) {
                throw new RuntimeException('Teacher email verification state was not updated.');
            }
        }
    } catch (Throwable $err) {
        error_log('Teacher settings update error: ' . $err->getMessage());
        $_SESSION['alert'] = [
            'error' => true,
    ...
    if (
        ...
        try {
            ...
            error_log('Teacher previous avatar delete error: ' . $err->getMessage());
        }
    }
    ...
    if ($emailChanged) {
        teacherPanelLogActivity($activityLogRepository ?? null, $userId, 'settings', 'Updated account settings and changed email address.');
        teacherPanelPrepareAccountVerification(
            $verifications,
            $tools,
            $userId,
            $email,
            (string) ($currentUser['username'] ?? ($_SESSION['username'] ?? 'Teacher'))
        );
        $_SESSION['alert'] = [
            'error' => false,
    ...
    $refreshedUser = $users->findSessionUser($userId);
    if ($refreshedUser) {
        teacherPanelRefreshSession($refreshedUser);
    }

    teacherPanelLogActivity($activityLogRepository ?? null, $userId, 'settings', 'Updated account settings.');
